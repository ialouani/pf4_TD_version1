...
//est tres belle.//
function bTI(l,v){
    if(l===tnil) return bF(v);//si tree vide.//
//sinn l!==tnil donc au moins un noeud avec 2 tnil.
    else{
	let value_of_node=l.va;
	if(v<=value_of_node){//place a gauche
	    return bN(value_of_node,bTI(lc(l),v),rc(l));}
	else if(v>value_of_node){
	    return bN(value_of_node,lc(l),bTI(rc(l),v));}}}
console.log(rc(lc(rc(bTI(aTree_bs,9)))));
let ttg=lc(rc(bTI(aTree_bs,9)));console.log(`${binaryTreeIsEmpty(lc(ttg))} \t \t ${rc(ttg).va}`);
console.log(lc(lc(bTI(aTree_bs,2))));
console.log("********************************************************");
console.log("********************************************************");
console.log("********************************************************EXO3");
//implementation rapide de map,reduce,filter tableaux Ecmascript pour linstant.
function map1(array,f){
//retourne en GROS : f(array).
    if(array.length===0) return [];
    else{
	return [f(array[0])].concat(map1(array.slice(1,array.length),f));}}
const array1 = [1, 4, 9, 16];
const map1_rslt = map1(array1,(elem,ind) => [elem * 2, ind]);
console.log(map1_rslt);
const _=require("underscore");
console.log(_.map(array1,(elem,ind)=>[elem*2,ind]));//prend par defaut
//ind tel que f(array[0])===f(array1,indc=0) donc donne [array1[0]*2,indc]..///
console.log(array1.map((elem,ind)=>[elem*2,ind]));
//directement comme methode des prototypes des tableaux instanciés.
function reduce1(array1,f){//array1!==[];//
    if(array1.length===0) return null;
    else{
	return f(array1[0],reduce1(array1.slice(1,array1.length),f));}}
function reduce13(array1,f,b){//array1!==[];//
    if(array1.length===0) return b;
    else{
	return f(array1[0],reduce13(array1.slice(1,array1.length),f,b));}}
console.log(reduce1([1,2,3,4],(acc,cV)=>acc+cV));
const array2=[1,2,3,4];const reducer=(acc,cV)=>acc+cV;
console.log(array2.reduce(reducer));
console.log(array2.reduce(reducer,5));
console.log(_.reduce(array2,reducer));
console.log(_.reduce(array2,reducer,5));
console.log(reduce13(array2,reducer,5));
function filtrer(tbl,f){
    if(tbl.length===1) {if(f(tbl[0])) return tbl; else return [];}
    else{
	//console.log(f(tbl[0]));
	if(f(tbl[0])===true) return [tbl[0]].concat(filtrer(tbl.slice(1,tbl.length),f));
	else return filtrer(tbl.slice(1,tbl.length),f);}}
const words = ['spray', 'limit', 'elite', 'exuberant', 'destruction', 'present'];
console.log(filtrer(words,word=>word.length<6));let stkr="erftg";
console.log(stkr.length);
function filtrer567(tbl,f){
    if(tbl.length===1) {if(f(tbl[0])) return tbl; else return [];}
    else{
	return (f(tbl[0]) && [tbl[0]].concat(filtrer567(tbl.slice(1,tbl.length),f))) || filtrer567(tbl.slice(1,tbl.length),f);}}
console.log(_.filter(words,word=>word.length<6));
console.log(words.filter(word=>word.length<6));
console.log(filtrer567(words,word=>word.length<6));
function cpid(th){
    if(th.length===1) return [th[0]];
    else return [th[0]].concat(th.slice(1,th.length));}
function pscl(t1,t2){
    let t4=t1.map((ei,indic)=>ei*t2[indic]);
    return t4.reduce((acc,cV)=>acc+cV);}
let ttgh1=[1,3,21];let ttgh2=[3,21,342];
console.log(pscl(ttgh1,ttgh2),3+3*21+21*342,ttgh1,ttgh2);
console.log(_.reduce(_.map(ttgh1,(ei,indic)=>ei*ttgh2[indic]),(acc,cV)=>acc+cV));
function map_evol(t,f,tl){
    if(t.length===1) return [ f(t[0], tl[0])];
    else{
	return [f(t[0],tl[0])].concat(map_evol(t.slice(1,t.length),f,tl.slice(1,tl.length)));}}
ttgh1=[1,3,21]; ttgh2=[3,21,342];
let txp=map_evol(ttgh1,(ei,ej)=>ei*ej,ttgh2);console.log(txp);
console.log(reduce13(map_evol(ttgh1,(ei,ej)=>ei*ej,ttgh2),(acc,cV)=>acc+cV,0));
console.log(reduce1(txp,(e,j)=>e+j));
function tn(n,k){//retourne tableau de 1 à n.n>=1 forcement.
    if(n===1) return [1];
    else{
    let tn_jet1=[n-k];
	return tn_jet1.concat(tn(n-1,k++));}}
console.log(tn(20,0));//en recursif terminal.//
console.log(Object.getOwnPropertyNames(Array));
function af(){
    return Array.from(arguments);}
let tghf=af(1,2,3);
console.log(tghf,typeof(tghf),typeof(txp),typeof(ttgh1));//..///////;.
console.log(Array.from);//[Function: from]..//
console.log(Object.getOwnPropertyNames(Array.from));
//comme arguments 
//facon de generer une instance tableau ECM5,6+++++/;;;>>>
//array.from([]) direct ou bien array.from(,)
//telque le 1er champ sera donné comme un objet et aussi le 2ieme tjrs
//car heritage.. => 1er champ,2ieme champ forment iterateur
//=>length indispensable=>1er champ==={length:???((a choisir))} puis
//pour le 2ieme une fonction formant un style de generation en BOUCLE
//le s courant interface liaions ++,<<<// commun sera index comme $2
//et le $1 vaudrAA _ valeur de reference arbitrairement fixee et verifiee
//qui est egal a 0.
console.log(Array.from([1,2,3,4,5])); 
console.log(Array.from({length: 5},(_,index)=>index+1));
function djn(n){
    return Array.from({length:n},(_,index)=>index+1);}//a reteniR.////
console.log(djn(17));
function div_ens(n){
    let h1=djn(n);
    return h1.filter(e=>n%e===0);}
console.log(div_ens(15));console.log(div_ens(19));
//avec nos fonctions + fonction _.filter de la bibli underscore:>>
function v2(n){
    let h1=tn(n,0);
    return _.filter(h1,e=>n%e===0);}
function v3(n){
    let h1=tn(n,0);
    return filtrer567(h1,e=>n%e===0);}
console.log(v2(19),v3(19));
console.log(v2(15),v3(15),tn(15,0));
function tna(n,k){
    if(n===0) return []; 
    return [k].concat(tna(n-1,k+1));}
console.log(tna(13,1));
function v2_bis(n){
    return _.filter(tna(n,1),e=>n%e===0);}
function v3_bis(n){
    return filtrer567(tna(n,1),e=>n%e===0);}
console.log(v2_bis(15),v3_bis(15));
console.log(v2_bis(20),v3_bis(20));
console.log("***********************************************");
//FIN DU TD5_PF./////